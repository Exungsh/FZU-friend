...
Page({
  ...
  async float_confirm() {
    ...
    if (!app.globalData.is_fzu) {
      ...
      var judgeInt = false;
      console.log("app.globalData.my_id", app.globalData.my_id)
      ...
      var xiaolei = await db.collection(res.data.small_tag).where({
        date: res.data.date,
        host: res.data.host,
        intro: res.data.intro,
        name: res.data.name,
      ...
      // console.log("res2.data[0]",res2.data[0])
      // console.log("res2.data[0].activities.length：", res2.data[0].activities.length)
      // console.log("dalei.data[0]._id", dalei.data[0]._id)

      await judge();
      async function judge() {
        for (var k = 0; k < app.globalData.my_activities.length; k++) {
          console.log("app.globalData.my_activities[k]", app.globalData.my_activities[k]);
          if (dalei.data[0]._id == app.globalData.my_activities[k]) {
            judgeInt = true;
            console.log("================app.globalData.my_activities[k]", app.globalData.my_activities[k])
          }
        }
      ...
      if (res.data.people_cnt < res.data.people_need) {
        if (judgeInt) {
          ...
          await db.collection(res.data.big_tag).doc(dalei.data[0]._id).update({
            data: {
              people_cnt: _.inc(1),
              people: _.push({
                "id": app.globalData.my_id,
                "name": app.globalData.my_name,
                "gender": app.globalData.my_sex,
          ...
          await db.collection(res.data.small_tag).doc(xiaolei.data[0]._id).update({
            data: {
              people_cnt: _.inc(1),
              people: _.push({
                "id": app.globalData.my_id,
          ...
          await db.collection('user').doc(app.globalData.my_id).update({
            data: {
              activities: _.push(dalei.data[0]._id)
            }
          });
          app.globalData.my_activities.push(dalei.data[0]._id);
